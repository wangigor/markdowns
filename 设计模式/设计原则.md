...
> 迪米特法则的英文翻译是Law Of Demeter(LOD):
>
> Each unit should have only limited knowledge about other units : only units 'closely' related to the current unit. Or: Each unit should only talk to its friends; Don't talk to strangers。
>
> 每个模块只应该了解那些与他关系密切的模块的有限知识。或者说，每个模块只和自己的朋友说话，不喝陌生人说话。

迪米特原则还有一个更加达意的名字，叫做最小知识原则「The Least Knowledge Principle」。

利用这个原则，能够帮我们实现代码的「高内聚、松耦合」，能够有效提高代码的可读性和可维护性，缩小功能改动导致的代码改动范围。 
...
- 抓取到页面，解析页面上的url连接，要把它组装成`Document`网页文档，为后续网页内容抽取、分词、索引等操作做准备。
- `Document`调用`HtmlDownloader.downloadHtml(url)`进行二级页面抓取。
- `HtmlDownloader`通过`NetworkTransporter`类进行底层网络请求，获取url请求的数据。

功能实现了，但是不够「好用」，有比较多的设计缺陷。

- **NetworkTransporter类**

  作为底层网络通信类，我们希望他的功能尽可能通用，而不只是服务与下载HTML。

  所以，我们不应该直接依赖太具体的发送对象`HtmlRequest`。

  这么来看依赖`HtmlRequest`层级太高，那就降低一下层级，让`NetworkTransporter`依赖`HtmlRequest`的下一层数据『`address`和`data`』。
...
- **HtmlDownloader类**
  ...
  修改了`NetworkTransporter`的`send()`的方法签名之后，这个`HtmlDownloader`类的调用代码就要进行修改。
...
- **Document类**
  ...
  - 构造函数中调用`downloader.downloadHtml()`逻辑复杂，耗时长，不应该放在构造函数中「会影响代码的可测试性」。
  - `HtmlDownloader`对象在构造函数中，每次都会new来创建。冗余。且违反了基于接口而非实现的编程思想。
  - 从业务含义来讲，`Document`网页文档没有必要依赖`HtmlDownloader`类，违反迪米特法则。
...
基于之前的单一职责原则，要拆分成两个类「序列化类和反序列化类」。

但是代码的可维护性就降低了，因为如果涉及序列化方式修改，就要同时修改两个类。

再有，前面讲的接口隔离原则，刚好能帮助不违背迪米特法则。就是抽两个接口「序列化和反序列化接口」。
...
迪米特法则的后半部分，就是基于单一职责原则和基于接口而非实现变成原则的集合。

> 抽象出两个接口，可能对于这个例子仅由两个接口来说，看起来有些设计过度了是不是？
>
> 其实不是，因为要对序列化添加更多功能时就能体现出来作用。
>
> 比如扩展对不同对象的序列化支持
>
> ```java
